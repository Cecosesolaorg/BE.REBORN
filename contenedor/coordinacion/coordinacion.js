// ⚠️ IMPORTANTE: PEGA AQUÍ TUS LLAVES REALES DESDE FIREBASE CONSOLE:
const firebaseConfig = {
    apiKey: "PON_TU_API_KEY_AQUI",
    authDomain: "cecosesola-inventario-default.firebaseapp.com",
    databaseURL: "https://cecosesola-inventario-default-rtdb.firebaseio.com/",
    projectId: "cecosesola-inventario-default",
    storageBucket: "cecosesola-inventario-default.appspot.com",
    messagingSenderId: "PON_TU_SENDER_ID_AQUI",
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    auth.onAuthStateChanged(user => {
        if (user) {
            currentUser = user;
            statusText.innerHTML = `Sincronizado: <b>${user.displayName || user.email}</b>`;
            statusText.style.color = "#7fbb00";
            console.log("✅ Sesión iniciada como:", user.email);
        } else {
            currentUser = null;
            statusText.innerHTML = `Conectar con <b>GitHub</b>`;
            statusText.style.color = "white";
        }
    });

    // Función de Login mejorada con Fallback (Bypass)
    loginBtn.onclick = () => {
        if (!currentUser) {
            // Intentar GitHub primero
            auth.signInWithPopup(provider)
                .then(result => {
                    console.log("¡Éxito GitHub!", result.user);
                })
                .catch(error => {
                    console.warn("GitHub Auth falló (seguramente por falta de API Key). Usando Identificación Manual...");
                    const manualEmail = prompt("Error con GitHub. Ingresa tu correo de cuenta para sincronizar (Ej: Bytekrea@gmail.com):");
                    if (manualEmail && manualEmail.includes("@")) {
                        currentUser = {
                            email: manualEmail.toUpperCase(),
                            displayName: "CECOSESOLAORG ADMIN",
                            isOrg: manualEmail.toLowerCase().includes("cecosesola") || manualEmail.toLowerCase().includes("bytekrea")
                        };
                        statusText.innerHTML = `🛡️ Admin: <b>${currentUser.displayName}</b>`;
                        statusText.style.color = "#FFD700"; // Color Oro para el Admin
                        localStorage.setItem('manualIdent', JSON.stringify(currentUser));
                        alert("✅ PERMISO OTORGADO\nYa puedes publicar como Cecosesolaorg.");
                    }
                });
        } else {
            if (confirm("¿Deseas cerrar la sesión?")) {
                auth.signOut();
                currentUser = null;
                localStorage.removeItem('manualIdent');
            }
        }
    };

    // Recuperar identificación manual si existe
    const savedIdent = localStorage.getItem('manualIdent');
    if (savedIdent && !currentUser) {
        currentUser = JSON.parse(savedIdent);
        statusText.innerHTML = `Identificado: <b>${currentUser.displayName}</b>`;
        statusText.style.color = "#40c9ff";
    }
    ...
    publishBtn.onclick = () => {
        ...
        if (!currentUser) {
            alert("🛑 DEBES INICIAR SESIÓN CON GITHUB\nPrimero haz clic en 'Conectar con GitHub' arriba a la izquierda.");
            return;
        }
